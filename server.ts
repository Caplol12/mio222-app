...
import { initDB, getAllUsers, syncUserRecord, updateUserPremiumStatus, updateUserStatus, findUserById, findUserByEmail, saveDB, getNextNumericId, DBUser } from "./server/db.ts";
...
async function startServer() {
  ...
  app.post("/api/auth/register", async (req, res) => {
    try {
      ...
      const existing = findUserByEmail(email);
      if (existing) return res.status(400).json({ error: "ایمیل تکراری است" });
      ...
      // Use syncUserRecord to ensure proper DB insertion
      ...
      res.json({ 
        token, 
        user: { id, numericId, name, email, picture: '', provider: 'local', isPremium: false, isAdmin: isAdminUser, status: 'active', createdAt: newUser.createdAt }
      });
    } catch (err) {
  ...
  app.post("/api/auth/google", async (req, res) => {
    try {
      ...
      if (!email) return res.status(400).json({ error: "ایمیل از گوگل دریافت نشد" });
      ...
      if (user) {
        ...
        if (!user.provider || user.provider === 'guest') user.provider = 'google';
        saveDB();
      } else {
