...
export default function LiveLogViewer({ title = 'سیستم لاگ زنده و عیب‌یابی Supabase', defaultExpanded = true, filterSource }: LiveLogViewerProps) {
        ...
        <div className="max-h-80 overflow-y-auto p-4 space-y-3 font-mono text-xs text-left dir-ltr bg-slate-950/70">
          {filteredLogs.length === 0 ? (
            <div className="text-center py-6 text-slate-500 dir-rtl text-sm">
              هیچ لاگی ثبت نشده است. هنگام ثبت‌نام یا ورود، لاگ‌ها به صورت زنده اینجا نمایش داده می‌شوند.
            </div>
          ) : (
