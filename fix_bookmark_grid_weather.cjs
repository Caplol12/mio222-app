...
// Remove TopWeatherWidget
const weatherWidgetStart = code.indexOf('function TopWeatherWidget() {');
const weatherWidgetEnd = code.indexOf('}', code.indexOf('return (', weatherWidgetStart)) + 2; // Roughly, but actually we need to find the matching brace.

// Better to use regex or string replace for the exact block.
const widgetCode = `function TopWeatherWidget() {
...
code = code.replace(widgetCode, '');

// Update default visibility
code = code.replace(
  "const defaultWidgetVisibility = {calendar: true, weather: true, notes: true, pomodoro: true, clock: true, search: true, board: true};",
  "const defaultWidgetVisibility = {calendar: true, notes: true, pomodoro: true, clock: true, search: true, board: true};"
);
...
// Remove usage
code = code.replace(
  "{widgetVisibility.weather && <TopWeatherWidget />}",
  ""
);

// We should also remove the comment
code = code.replace(
  "{/* Weather & Clock Top Widgets */}",
  "{/* Clock Top Widget */}"
);
